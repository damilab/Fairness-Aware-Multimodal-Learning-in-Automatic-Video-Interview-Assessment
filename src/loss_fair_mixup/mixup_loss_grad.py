...
class MixupLossGrad(nn.Module):
    ...
    def _cal_loss_grad(
        ...
        inputs_mix = group_image_list[0] * lam[0]
        for i in range(1, len(group_len_list)):
            inputs_mix += group_image_list[i] * lam[1]
        inputs_mix = inputs_mix.requires_grad_(True)
        ops, _ = model(inputs_mix)
        ops = ops.sum()
        ...
        for i in range(0, len(group_len_list) - 1):
            for j in range(i + 1, len(group_len_list)):
                inputs_1 = group_image_list[i]
                inputs_0 = group_image_list[j]
